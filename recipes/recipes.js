...
function generateRecipeTemplate(recipe) {
    return `
        <section class="recipe">
            <img src="${recipe.image}" alt="${recipe.name} Dessert">
            <div class="recipe-content">
                ...
                <span
                    class="rating"
                    role="img" aria-label="Rating: ${recipe.rating} out of 5 stars"
                    >
                ${generateRatingStarsTemplate(recipe.rating)}
                </span>
...
// Create a template function to generate the markup to display the tags. It should expect a list of tags as a parameter.
function generateTagsTemplate(tags) {
    return tags.map(tag => `<span class="tag">${tag}</span>`).join('')
}

// Create another template function to generate the correct rating stars. This function will expect a number to represent how many stars. Make sure to retain the class and aria info on the ratings!
function generateRatingStarsTemplate(rating) {
    const filledStars = '⭐'.repeat(rating)
    const emptyStars = '☆'.repeat(5 - rating)
    return `
        <span class="rating" role="img" aria-label="Rating: ${rating} out of 5 stars">
            <span aria-hidden="true" class="icon-star">${filledStars}</span>
            <span aria-hidden="true" class="icon-star-empty">${emptyStars}</span>
        </span>
    `
}

// Using the random recipe function, create an init function the should run when the page loads to render out a random recipe.
function init() {
    const randomRecipe = getRandomRecipe(recipes);
    const recipeHTML = generateRecipeTemplate(randomRecipe);
    document.querySelector('.recipes-list').innerHTML = recipeHTML;
}

// Test to make sure everything is working so far.
window.onload = init
...
function renderRecipes(recipesList) {
    ...
    recipesContainer.innerHTML = recipesList.map(generateRecipeTemplate).join('');
}
